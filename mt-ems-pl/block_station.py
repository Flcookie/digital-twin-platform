    ...
    while True:
        ...
        if component_status == "STOP":
            ...
            if not is_part_ready:
                ...
                pusher_motor.run_to_position(pusher_motor_start_position, block=False)
                ...
                component_event = common.build_event(COMPONENT_ID, part_id, "PROCESS")
                mqtt_topic = common.render_topic("component_event", COMPONENT_ID, "all")
                mqtt_payload = common.serialize_object(component_event)
                mqtt_client.publish(mqtt_topic, payload=mqtt_payload, qos=2)
                ...
                if random.random() < FAILURE_PROBABILITY:
                    component_event = common.build_event(COMPONENT_ID, part_id, "FAIL")
                    mqtt_topic = common.render_topic(
                ...
                part_uid = machine_sensor.read_passive_target(timeout=float("inf"))
                is_part_updated = specific.update_part(machine_sensor, part_uid, SCHEMA)
                ...
                mqtt_client.publish(mqtt_topic, payload="BOOK", qos=2)
                common.sleep(
                    condition=lambda: (
                ...
                if not is_request_accepted:
                    component_event = common.build_event(COMPONENT_ID, part_id, "BLOCK")
                    mqtt_topic = common.render_topic(
                        "component_event", COMPONENT_ID, "all"
                    )
                    mqtt_payload = common.serialize_object(component_event)
                    ...
                    common.sleep(
                        condition=lambda: (
                            system_status == "START" and not is_request_accepted
                        )
                    )
                    if system_status == "STOP":
                        continue
