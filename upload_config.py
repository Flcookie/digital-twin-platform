...
LOCAL_CONFIG_PATHS = CONFIG["local_config_paths"]

for hostname in CONTROLLER_HOSTNAMES:
    config_paths = LOCAL_CONFIG_PATHS[hostname]
    ...
    for config_path in config_paths:
        local_path = os.path.normpath(os.path.join(LOCAL_FOLDER_PATH, config_path))
        remote_path = REMOTE_FOLDER_PATH + "/" + os.path.basename(config_path)
        sftp_session.put(local_path, remote_path)
        print(
            "Config " + local_path + " uploaded to folder " + REMOTE_FOLDER_PATH
