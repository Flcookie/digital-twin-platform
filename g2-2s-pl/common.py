...
def safe_set_system_time(payload, log_prefix=""):
    """Set system time via sudo date -s. Validates payload to prevent command injection."""
    if not payload or not re.match(r"^[\d\s\-:TZ\.]+$", payload.strip()):
        if log_prefix:
            print("{} Ignored invalid system_time payload".format(log_prefix))
        return False
    try:
        ...
        if log_prefix:
            print("{} set_system_time error: {}".format(log_prefix, e))
        return False
...
def notify_status_changed():
    """Call when MQTT/system_status changes. Wakes sleep() early (event-driven)."""
    _status_event.set()


def sleep(duration=float("inf"), condition=lambda: True):
    """Event-driven wait: use Event.wait() instead of busy-polling when condition may change via MQTT."""
    end_time = time.time() + duration
    while time.time() < end_time and condition():
        _status_event.clear()
        remaining = min(SLEEP_TIME_RESOLUTION, max(0.01, end_time - time.time()))
        _status_event.wait(timeout=remaining)
...
class Sensor(pn532_uart.PN532_UART):
    ...
    def set_property(self, uid, schema, name, value):
        ...
        if count < WRITE_ENDURANCE:
            ...
            if block_index >= 64:
                raise RuntimeError("No more spare blocks")
